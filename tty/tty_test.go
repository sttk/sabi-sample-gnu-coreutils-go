...
func (dax mapDax) GetTtyName(fd int) (string, sabi.Err) {
	if dax.m["error"] == "notty" {
		return "", sabi.ErrBy(StdinIsNotTty{})
	}
	return dax.m["ttyname"].(string), sabi.Ok()
}

func (dax mapDax) PrintModeError(err sabi.Err) sabi.Err {
	dax.m["print"] = err
	return sabi.Ok()
}
